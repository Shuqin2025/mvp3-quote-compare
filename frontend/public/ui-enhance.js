...
(() => {
  ...
  const getApiBase = () => {
    try {
      ...
      return (u.searchParams.get('api') || '').replace(/\/+$/,''); // e.g. https://yunivera-mvp2-cwyr.onrender.com
    } catch { return ''; }
  };
  ...
  const mountToast = () => {
    ...
    if (!toastEl.parentNode) {
      toastEl.style.cssText = 'margin:10px 0;padding:8px 12px;border-left:4px solid #0ea5e9;background:#f0f9ff;display:none;';
  ...
  const ensureTable = () => {
    ...
    if (!table) {
      table = create('table', { class: 'mvp3-table', style: 'width:100%;border-collapse:collapse;font-size:14px;table-layout:fixed' });
  ...
  async function probeHealth(apiBase) {
    const cand = ['/api/health','/health','/api/healthz','/healthz'];
    for (const p of cand) {
  ...
  const fetchCatalog = async () => {
    try {
      ...
      // 健康自检（只做一次请求，不阻塞主流程）
      probeHealth(api).then(h => { if (h.ok) console.log('[health]', h.url, h.text); });
